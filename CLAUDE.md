...
**Key components:** `SessionList`, `Timeline`, `TimelineNode`, `ToolPairNode`, `MacroNodeCard`, `StatusIndicator`, `SuggestionButtons`, `PromptInput`, `MarkdownContent`, `AISparkle`, `AuthProvider`, `ThemeProvider`, `ToastProvider`, `NavBar`, `SkeletonLoader`, `AgentSelector`, `AgentBadge`, `RoleBadge`, `RoleSelector`.
...
- **Role color convention**: SDE=accent-blue, QA=accent-green, PM=accent-purple, unknown=accent-amber. Defined in `role-colors.ts` (single source of truth), imported by `RoleBadge.tsx` and `RoleSelector.tsx`. Pattern-matches `AgentBadge`/`AgentSelector` from `AgentSelector.tsx`.
...
- **Role side-effect imports**: Modules using `getRole()`, `getAllRoles()`, or `getBuiltinRoles()` must import all role modules (`roles/role-sde.js`, `roles/role-qa.js`, `roles/role-pm.js`) as side-effects. Currently done in `plan-executor.ts`, `plan-generator.ts`, `plan-coordinator.ts`, `plan-routes.ts`, and `routes.ts`. Same pattern as agent runtimes.
...
- **Shared frontend test utilities**: `test-utils.tsx` provides `renderWithProviders` (ToastProvider wrapper), mock factories (`makeMockBlueprint`, `makeMockNode`, `makeMockExecution`, `makeMockInsight`), and `mockAllApiDefaults()`. `test-setup.ts` includes a global `BroadcastChannel` mock (jsdom doesn't provide one).
