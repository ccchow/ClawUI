...
Next.js 14, React 18, Tailwind CSS 3, TanStack Query v5, TanStack Virtual v3, dark/light theme via `next-themes`. See [`docs/FRONTEND-PATTERNS.md`](docs/FRONTEND-PATTERNS.md) for detailed UI conventions.
...
- **Autopilot test mock ordering with reflections**: `mockRunSession` is shared between decision calls, reflection calls (`reflectAndUpdateMemory`), and suggestion generation calls (`generateBlueprintSuggestions`). Reflections happen every `REFLECT_EVERY_N` (5) iterations, on pause, and on error. Suggestions are generated once post-loop (after final reflection, before global memory). Tests with 5+ loop iterations must insert reflection response mocks at the right positions. For sequential `mockResolvedValueOnce` chains, add a `mockResolvedValueOnce('[]')` for the suggestions call between reflection and global memory mocks. For persistent `mockImplementation`, filter on prompt content: reflections contain `"reflecting"`, global memory contains `"global autopilot"` or `"global strategy"`, suggestions contain `"generating actionable suggestions"`.
- **AI operations in plan-operations.ts**: `enrichNodeInternal`, `reevaluateNodeInternal`, `splitNodeInternal`, `smartDepsInternal`, `reevaluateAllInternal` are extracted from route handlers. Called by `plan-routes.ts` in manual mode only. In autopilot/FSD mode, these endpoints create a user message via `createAutopilotMessage` and call `triggerUserAgent` instead — the User Agent translates the request into API operations. `runWithRelatedSessionDetection` helper also lives here.
- **Two-layer autopilot architecture**: User messages are handled by the **User Agent** (`user-agent.ts`) — a full Claude CLI session that calls ClawUI REST API endpoints (same as frontend). Node execution is handled by the **FSD Loop** (`autopilot.ts`) — a pure node execution engine. The User Agent is equivalent to a human user on the web UI. The FSD Loop yields (breaks) when unacknowledged user messages exist, letting the User Agent process them first.
- **User Agent trigger flow**: `triggerUserAgent(blueprintId)` in `user-agent.ts` enqueues `handleUserMessage` which runs a `runSession` Claude CLI session. After the session, messages are acknowledged and `triggerFsdLoopIfNeeded` is called. Used by: POST /messages, enrich/split/smart-deps/reevaluate-all/generate endpoints in autopilot mode.
- **FSD Loop trigger flow**: `triggerFsdLoopIfNeeded(blueprintId)` in `autopilot.ts` checks if blueprint is in autopilot/FSD mode and no loop is running, then enqueues `runAutopilotLoop`. Called by User Agent after processing messages. Resume-from-pause and mode-switch call `runAutopilotLoop` directly.
- **User Agent calls REST API, not internal functions**: The User Agent runs a Claude CLI session that calls the same HTTP endpoints as the frontend via bash/curl. It does NOT import or call internal functions like `createMacroNode` or `splitNodeInternal`. Auth is passed via `?auth={token}` query parameter.
- **Autopilot tool palette (FSD Loop only)**: `autopilot.ts` uses read tools (`get_node_titles`, `get_node_details`, `get_node_handoff`), and `run_direct(prompt)` for simple tasks. No user message tools (no `acknowledge_message`, `read_user_messages`, `send_message`). Two execution paths: **run_direct** for simple/transactional tasks vs **create_node + run_node** for complex engineering tasks. `AutopilotNodeState` is lightweight (no `description` or `suggestions` — fetched on-demand).
...
- **BlueprintChat virtual scrolling**: `BlueprintChat` uses `@tanstack/react-virtual` with a dual-path render: items below `VIRTUALIZATION_THRESHOLD` (100) render normally with `space-y-3`; above it, `useVirtualizer` renders only visible items with absolute positioning. Both paths share `renderChatItem()`. Tests use small data sets (non-virtual path) to avoid jsdom's lack of element dimensions. The threshold and `DisplayItem` type are exported for testing.
- **Circular dependency: autopilot ↔ plan-executor**: `autopilot.ts` imports from `plan-executor.ts`. If `plan-executor.ts` needs to call autopilot (e.g. recovery), use dynamic `import("./autopilot.js")` to avoid circular import. Similarly, `user-agent.ts` uses dynamic `import("./autopilot.js")` for `triggerFsdLoopIfNeeded` to avoid circular dependency.
- **Windows cross-platform support**: `naiveDecodePath()` in `db.ts` handles Windows drive letters (`Q--src-ClawUI` → `Q:\src\ClawUI`). `isProcessAlive()` in `cli-utils.ts` uses `tasklist` on Windows instead of `process.kill(pid, 0)`. Dev redeploy uses `spawn({ detached: true, windowsHide: true })` instead of `nohup`. Path splitting uses `/[\\/]/` regex to handle both separators.
