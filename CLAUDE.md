...
- **expect for TTY (Unix)**: Claude CLI requires TTY — uses `expect` (auto-detected via `EXPECT_PATH`) with `set stty_init "columns 2000"`
- **Windows support**: On Windows, Claude CLI runs directly via `node cli.js` without TTY/expect. Path encoding uses `encodeProjectPath()` from `cli-utils.ts` (handles `/` and `\`, strips drive colons). `decodeProjectPath()` detects Windows drive letters. Shell scripts have `.mjs` equivalents in `scripts/`.
...
- **Windows `.cmd` shim**: `spawn("npx", ...)` fails on Windows — use `spawn("node", [cliJsPath, ...])` via `CLAUDE_CLI_JS` in `config.ts`
- **Windows path encoding**: Use `encodeProjectPath()` from `cli-utils.ts`, not naive `path.replace(/\//g, "-")`
...
- `CLAUDE_PATH` — Path to Claude CLI binary (auto-detected: Unix checks `~/.local/bin/claude`, `/usr/local/bin/claude`, then PATH; Windows checks `%APPDATA%\npm\claude.cmd`, then `where.exe`)
- `EXPECT_PATH` — Path to `expect` binary, Unix only (auto-detected: checks common locations then PATH; empty on Windows)
